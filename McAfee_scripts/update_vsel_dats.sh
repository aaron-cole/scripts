...
#SSH TO SERVER
    ssh -t $f "sudo /etc/init.d/cma restart; tar xvf $DATFILE; sudo mv -f ./*.dat /opt/NAI/LinuxShield/engine/dat/ ; sudo chown -R root:root /opt/NAI/LinuxShield/engine/dat/; rm -rf $DATFILE; sudo /etc/init.d/nails restart; sudo /opt/McAfee/cma/bin/cmdagent -P"
    if [[ $? -ne 0 ]]; then
