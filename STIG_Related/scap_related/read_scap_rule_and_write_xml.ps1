...
foreach ($rule in $rules) {
    # Extract the rule ID
    $ruleId = $rule.GetAttribute("id") -replace '^.*SV-' , 'V-' -replace 'r.*_rule',''
#    $test = $rule.GetAttribute("id") -replace '^.*SV-' , 'V-' -replace 'r.*_rule',''
#    $ruleid = $rule.id -replace '^.*SV-' , 'V-' -replace 'r.*_rule',''
