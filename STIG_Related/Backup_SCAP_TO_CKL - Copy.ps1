...
if (($scapfile -ne $null) -or ($scapdir -eq $null))
  { if ($scapfile -eq $null) 
    ...
    $scap_file_to_use = $scapfile
    while (!(Test-Path $scap_file_to_use -PathType Leaf) )
    { Write-Host "SCAP File Does not Exist or NOT PROVIDED`n"
      $scap_file_to_use = Read-Host "Full Path to SCAP File"      
    }
  }
...
function Save-CKL { 
  ...
  Write-Host "" 
  Write-Host "###  The following file was created $NEW_CKL_FILE"
}
###################
# Variables
...
###################
#Variables for SCAP File
[xml]$scapdata = Get-content -Path "$scap_file_to_use"
$scap_computing = $scapdata.'asset-report-collection'.assets.asset.'computing-device'
#$scapdata.'asset-report-collection'.assets.asset.'computing-device'.fqdn
#$scapdata.'asset-report-collection'.assets.asset.'computing-device'.hostname
$scapips = $scap_computing.connections.connection.ChildNodes.'ip-v4'
$scapmacs = $scap_computing.connections.connection.'mac-address'

#May only Require this for facts
#Scap Test Results Section
...
#$scap_scanner_name = ($scap_test_results.'target-facts'.fact | ? { $_.name -eq "urn:xccdf:fact:scanner:name" }).'#text'
#$scap_scanner_version = ($scap_test_results.'target-facts'.fact | ? { $_.name -eq "urn:xccdf:fact:scanner:version" }).'#text'
...
#$SCAPIPS = $scapdata.'asset-report-collection'.assets.asset.'computing-device'.connections.connection.ChildNodes.'ip-v4'
