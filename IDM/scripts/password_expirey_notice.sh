...
# grepping memberUid for the group you want and piping to awk results in a list of users
 USERLIST=$(ldapsearch -x -b cn=$GRP,cn=groups,cn=compat,dc=myidm,dc=local | grep memberUid | awk '{print $2}')
...
#This way is slow - that's the only reason it's not the main search
 if [ -z "$USERLIST" ]; then
  USERLIST=$(ldapsearch -Y GSSAPI -LLL -Q -b cn=$GRP,cn=groups,cn=accounts,dc=myidm,dc=local | grep "^member:" | awk '{print $2}'  | cut -f1 -d"," | cut -f2 -d"=")
 fi
...
# create the email content
   ...
   mailbody="You are receiving this email because the password for your $USER account expires on $(date -d $EXPIRYDATE +%D). Policy requires that all user accounts change their password every 60 days. 
...
If your account becomes disabled, you will have to submit a WO to have your account unlocked.
