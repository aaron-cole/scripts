...
if ! grep "6." /etc/redhat-release >> /dev/null; then
 echo "This can only be ran on a RHEL6 Machine"
 exit 1
fi
...
REPOLIST="rhel-x86_64-server-6-thirdparty-oracle-java rhel-x86_64-server-6-rhdirserv-9 rhel-x86_64-server-extras-6 rhel-x86_64-server-ha-6 rhn-tools-rhel-x86_64-server-6 rhel-x86_64-server-supplementary-6 rhel-x86_64-server-rs-6 rhel-x86_64-server-optional-6 rhel-x86_64-server-6"
...
for repo in $REPOLIST; do
 reposync --gpgcheck -l -n --repoid=$repo --download_path=/u02/reposync
done
